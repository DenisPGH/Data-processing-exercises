...
with open('info.csv', mode='w',newline='') as job_file:
    ...
    for each_job_ind in range(len(result['documents'])):
        ...
        publication_date = result["documents"][each_job_ind]['preview']
        ...
        jobs_writer.writerow([title,publication_date,place,is_active])


#print(len(result["documents"][0]))
link=result["documents"][0]['_links']['detail_de']['href']
title=result["documents"][0]['title']
id=result["documents"][0]['company_id']
preview=result["documents"][0]['publication_date']
publication_date=result["documents"][0]['preview']
